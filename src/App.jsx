  ...
  return (
    <div style={{ position:"relative", width:size, height:size, margin:"0 auto" }}>
      ...
      <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", textAlign:"center", pointerEvents:"none" }}>
        <div style={{ fontSize:16, fontWeight:700, color:"#e8eaf6", lineHeight:1 }}>{hrs>0?`${hrs}h `:""}{mins}m</div>
...
function WeeklyReport({ records, notes }) {
  ...
  const weekData = weekDays.map((date, i) => {
    ...
    return { date, label: DAY_LABELS[i], total, prod, prodRate, recs };
  });
  ...
  const catTotals = CATEGORIES.map(cat => ({
    ...
    minutes: weekDays.reduce((s, date) => {
      return s + (records[date]||[]).filter(r=>r.cat===cat.id).reduce((a,r)=>a+r.min, 0);
    }, 0)
  ...
  if (weekTotal === 0) return (
    <div style={{ background:"#13152a", borderRadius:14, padding:"16px", marginBottom:10, textAlign:"center", color:"#3a4060", fontSize:13 }}>
      이번 주 기록이 없어요
    </div>
  );

  return (
    <div style={{ background:"#13152a", borderRadius:14, padding:"16px", marginBottom:10 }}>
      <div style={{ fontSize:11, color:"#4a5270", letterSpacing:1, marginBottom:14 }}>이번 주 분석</div>

      {/* 요일별 바 */}
      <div style={{ display:"flex", alignItems:"flex-end", gap:4, height:80, marginBottom:8 }}>
        {weekData.map((d, i) => {
          const height = d.total > 0 ? Math.max((d.total/maxTotal)*70, 6) : 0;
          ...
          return (
            <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
              <div style={{ width:"100%", height:70, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
                <div style={{ width:"70%", height: height||4, background: d.total>0?color:"#1e2038", borderRadius:4, transition:"height .4s", opacity: d.total>0?1:0.3 }}/>
              </div>
      ...
      {/* 주간 합계 */}
      <div style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderTop:"1px solid #1e2038", borderBottom:"1px solid #1e2038", marginBottom:14 }}>
      ...
      <div style={{ marginBottom:14 }}>
        <div style={{ fontSize:11, color:"#4a5270", marginBottom:8 }}>카테고리별</div>
        {catTotals.slice(0,4).map(cat => {
...
export default function App() {
  ...
  const addRecord = () => {
    ...
    const newRecords = { ...(isSample ? {} : records), [date]: [...((isSample ? {} : records)[date]||[]), newRec] };
    const newHist = historyDates.filter(d=>d!==date&&!Object.keys(SAMPLE_RECORDS).includes(d));
    const updated = newHist.includes(date) ? newHist : [date,...newHist].sort((a,b)=>b.localeCompare(a)).slice(0,90);
    setRecords(newRecords); setHistoryDates(updated); save(newRecords, updated);
  ...
  const deleteRecord = (idx) => {
    ...
    setRecords(newRecords); save(newRecords);
  };

  const saveNote = (date, text) => {
    const newNotes = { ...(isSample ? {} : notes), [date]: text };
    setNotes(newNotes);
  ...
  const prevCal = () => { const d=new Date(year,month-2,1); setCalMonth(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`); };
  const nextCal = () => { const d=new Date(year,month,1); const next=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`; if(next<=todayKey().slice(0,7)) setCalMonth(next); };

  // 선택된 날짜 상세
  const histRecs = selectedHistDate ? (records[selectedHistDate]||[]) : [];
  const histCatTotals = CATEGORIES.map(cat => ({ ...cat, minutes: histRecs.filter(r=>r.cat===cat.id).reduce((s,r)=>s+r.min,0) }));
        ...
        <div style={{ padding:"14px 16px" }}>
          ...
          <div style={{ display:"flex", gap:8, marginBottom:14 }}>
            <button onClick={backupData} style={{
              flex:1, background:"#1e2038", border:"none", borderRadius:10, padding:"11px",
              color:"#8892b0", fontSize:12, fontWeight:600, cursor:"pointer",
              display:"flex", alignItems:"center", justifyContent:"center", gap:6
            }}>⬇️ 백업 저장</button>
            <label style={{
              flex:1, background:"#1e2038", borderRadius:10, padding:"11px",
              color:"#8892b0", fontSize:12, fontWeight:600, cursor:"pointer",
              display:"flex", alignItems:"center", justifyContent:"center", gap:6
            }}>
              ⬆️ 백업 복원
          ...
          <div style={S}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", marginBottom:6 }}>
              {["일","월","화","수","목","금","토"].map((d,i)=>(
                <div key={d} style={{ textAlign:"center", fontSize:11, color: i===0?"#FF6B6B66":i===6?"#4F7CFF66":"#4a5270", padding:"4px 0" }}>{d}</div>
              ))}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3 }}>
              ...
              {Array.from({length:daysInMonth},(_,i)=>i+1).map(d=>{
                ...
                return (
                  <button key={d} onClick={()=>!isFuture && setSelectedHistDate(isSelected ? null : dateStr)} style={{
                    aspectRatio:"1", borderRadius:8, border:"none", cursor: isFuture?"default":"pointer",
                    background: isSelected ? "#4F7CFF" : color ? color+"22" : "#1e2038",
                    display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:2,
                    outline: isToday ? "2px solid #4F7CFF44" : "none", outlineOffset:1, position:"relative"
                  }}>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:14, marginTop:12 }}>
              {[["#00C48C","생산 70%+"],["#FFB547","40~69%"],["#FF6B6B","~39%"]].map(([c,l])=>(
          ...
          {selectedHistDate && (
            <div>
              <div style={{ fontSize:13, fontWeight:600, color:"#c8d0e8", marginBottom:10, paddingLeft:4 }}>
